...
<!--parity sapre emered bcone and hr div -->
  ...
  <th><button onclick="location.href='bcon_win'"  style="color:#2196f3;height:40px;width:50%;float:right;background:none;border:0px;font-weight:bold">Bcone</button></th>
  <th><button onclick="location.href='emerd_win'"  style="height:40px;width:50%;float:right;background:none;border:0px;font-weight:bold">Emerd</button></th>
    ...
    <hr id="hr1" style="margin-top:0px;color:yellow;border: 1px solid #2196f3;width:25%;float:left;margin-left:55%">
...
<div style="width:100%;height:50px;margin-top:0px;background:white">
    ...
    <br>
    <i style="font-size:15px;margin:10px;">  <i id="f_my" class="far fa-trophy-alt" style="font-size:25px;color:#696969;color:grey"></i> Bcone Record</i>
    </p>
    </div>

    <?php
  
  $parity = mysqli_query($conn,"SELECT * FROM bcon_records order by period DESC LIMIT 10");
...
<div style="width:100%;height:40%;margin-top:0px;background:white;overflow:scroll">
  ...
  <center>My Bcone Record</center>
        
 <hr>
  <?php
  
    $selecting = mysqli_query($conn,"SELECT * FROM transactions where wallet_id ='$id'  && section=3 order by transaction_id DESC ");
    ...
    if($wallet_result['available_balance']>=$amount)
    {
     ...
     else              
      {
            ...
            //inserting into transactions
              $sql = mysqli_query($conn,"INSERT INTO transactions VALUES('$t_id','$current_period','$select','$amount','$id','0','3')");
              ...
              {     
                ...
                echo "<script>window.location.href='bcon_win';</script>";
              }
